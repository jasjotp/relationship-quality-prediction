...
To see how our model did on predicting relationship quality based on the training data, we plot a confusion matrix @fig-cm-train. We see that for respondents with excellent relationship quality, the model only correctly predicts `{python} round(excellent_recall_train * 100, 1)`% of relationships that have excellent relationship quality correctly (Recall = `{python} excellent_recall_train`). Of all relationships that are predicted to have excellent relationship quality,the model correctly predicts `{python} round(excellent_precision_train * 100, 1)`% of them (Precision = `{python} round(excellent_precision_train, 3)`).
...
To see how our model did on predicting relationship quality based on the testing set @fig-cm-test, we plot a confusion matrix for predictions on the test set. We see that for respondents with excellent relationship quality, the model only correctly predicts `{python} round(excellent_recall_test * 100, 1)`% of relationships that have excellent relationship quality correctly (Recall = `{python} round(excellent_recall_test, 3)`). Of all relationships that are predicted to have excellent relationship quality,the model correctly predicts `{python} round(excellent_precision_test * 100, 1)`% of them (Precision = `{python} round(excellent_precision_test, 3)`).
